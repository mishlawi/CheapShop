...
def getProdutosPagina(link):
    ...
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.76 Safari/537.36',"Accept-Language": "en-US,en;q=0.5"}
    html = requests.get(link,headers=headers).text
    soup = BS(html,"html.parser")

    navbar = soup.find('nav',class_='top_menu')
    categorieselem = navbar.find_all(lambda tag: tag.name == 'a' and tag.get('class') == ['top_menu-item'],recursive=False)
    ...
    for categorylink in categorieslinks:
        scrappingWorker = ThreadWithReturnValue(target=processCategory, args=(categorylink,), name=categorylink.split('/')[-1])
...
def processCategory(categoryLink):
    ...
    totalcategoryproducts = int(soup.find('div',class_='c12 c4-xl-up tc').find('h2').text.strip().split(' ')[0].replace('.',''))
    print('# total products',totalcategoryproducts, categoryLink.split('/')[-1])
    ...
    paginationcontrols = paginationdiv.find('div',class_='pagination-controls c12')
    totalpages = paginationcontrols.find('li',id='pagination-current').text.strip().split(' ')[-1]
    ...
    for _ in range(int(totalpages)):
        ...
        productselems = productsgrid.find_all('div',class_=re.compile('grid-item product_tile _retro _supermarket')) #'grid-item product_tile _retro _supermarket _nostock dataholder js-product' --apanhar produtos sem stock?
        ...
        for productelem in productselems:
            ...
            pricediv = productelem.find('div',class_='product_tile-price_holder')
            ...
            if pricediv.find('div',class_='prices-price _pum'):
                ppu = pricediv.find('div',class_='prices-price _pum').text.strip()
            else:
                ppu = None
